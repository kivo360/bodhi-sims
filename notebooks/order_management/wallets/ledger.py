...
Transaction = namedtuple('Transaction', [
    'poid', 'step', 'source', 'target', 'memo', 'amount', 'free', 'locked',
    'locked_poid'
])
...
class Ledger:
    ...
    def commit(self, wallet: 'Wallet', quantity: 'Quantity', source: str,
               target: str, memo: str) -> None:
        ...
        locked_poid_balance = None if poid not in wallet.locked.keys(
        ) else wallet.locked[poid]

        transaction = Transaction(poid, wallet.exchange.clock.step, source,
                                  target, memo, quantity, wallet.balance,
                                  wallet.locked_balance, locked_poid_balance)
