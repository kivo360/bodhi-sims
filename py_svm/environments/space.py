...
# Standard Library
import math
from typing import (Any, Set, cast, Dict, List, Tuple, Union, TypeVar, Callable,
                    Iterable, Iterator, overload, Sequence)
...
# For Mypy
from py_svm.environments.agent import Agent
...
class Grid:
    ...
    def __getitem__(
        self, index: Tuple[int | slice,
                           int | slice]) -> GridContent | List[GridContent]:
        ...
    ...
    def neighbor_iter(self,
                      pos: Coordinate,
                      moore: bool = True) -> Iterator[Agent]:
        ...
        warn("`neighbor_iter` is deprecated in favor of `iter_neighbors` "
    ...
    def iter_cell_list_contents(
            self, cell_list: Iterable[Coordinate]) -> Iterator[Agent]:
    ...
    @accept_tuple_argument
    def get_cell_list_contents(self,
                               cell_list: Iterable[Coordinate]) -> List[Agent]:
...
class MultiGrid(Grid):
    ...
    def iter_cell_list_contents(
            self,
            cell_list: Iterable[Coordinate]) -> Iterator[MultiGridContent]:
...
class HexGrid(Grid):
    ...
    def iter_neighborhood(self,
                          pos: Coordinate,
                          include_center: bool = False,
                          radius: int = 1) -> Iterator[Coordinate]:
        """Return an iterator over cell coordinates that are in the
        neighborhood of a certain point.
        ...
        def find_neighbors(pos: Coordinate, radius: int) -> None:
            ...
            if x % 2 == 0:
                ...
                adjacent += [(x - 1, y), (x - 1, y - 1), (x + 1, y),
                             (x + 1, y - 1)]

            if self.torus is False:
                adjacent = list(
                    filter(lambda coords: not self.out_of_bounds(coords),
                           adjacent))
            else:
...
class ContinuousSpace:
    ...
    def get_neighbors(self,
                      pos: FloatCoordinate,
                      radius: float,
                      include_center: bool = True) -> List[Agent]:
        ...
        neighbors = [
            ...
            for x in idxs
            if include_center or dists[x] > 0
        ]
        return neighbors

    def get_heading(self, pos_1: FloatCoordinate,
                    pos_2: FloatCoordinate) -> FloatCoordinate:
    ...
    def get_distance(self, pos_1: FloatCoordinate,
                     pos_2: FloatCoordinate) -> float:
...
class NetworkGrid:
    ...
    def get_neighbors(self,
                      node_id: int,
                      include_center: bool = False) -> List[int]:
    ...
    def iter_cell_list_contents(self,
                                cell_list: List[int]) -> List[GridContent]:
        """Returns an iterator of the contents of the cells
        identified in cell_list."""
        list_of_lists = [
